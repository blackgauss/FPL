...
def build_season_tree_dense(
    base: Path, season: str = "2025-2026",
    n_players: int = 20, n_gws: int = 4, n_clubs: int = 5,
) -> Path:
    """A denser synthetic season: `n_players` across all 4 positions and
    `n_gws` gameweeks, split over `n_clubs`. Feasible for a full 15-man squad
    (>= needed per position, club spread so ≤3/club works). Same layout/traps
    as the toy tree but with breadth for integration tests."""
    season_dir = base / season
    gw_dir = season_dir / "By Gameweek"
    season_dir.mkdir(parents=True, exist_ok=True)
    ...
    for gw in range(1, n_gws + 1):
        ...
        (folder / "matches.csv").write_text(
            ...
            f"{gw}.0,2025-08-1{gw}T16:30:00+00:00,{clubs[0]}.0,2050.0,"
            f"{1 + gw},0,{clubs[1]}.0,2000.0,True,m{gw},u{gw},prem\n",
            encoding="utf-8")
        stats = ["id,gw,web_name,second_name,status,total_points,minutes,goals_scored,"
                 "assists,bonus,bps,saves,starts,now_cost,form,ep_next,ep_this,"
                 "selected_by_percent"]
        for i in range(n_players):
            stats.append(
                f"{i+1},{gw},P{i},P{i},a,{(i + gw) % 9},{45 + (i % 3) * 30},"
                f"{(i + gw) % 3},{(i + gw) % 2},0,{(i + gw) % 20},0,1,"
                f"{4.0 + 0.5 * (i % 8)},3.0,4.0,4.0,2.0")
        (folder / "player_gameweek_stats.csv").write_text(
            "\n".join(stats) + "\n", encoding="utf-8")
        mstat = ["player_id,match_id,minutes_played,goals,assists,xg,xa"]
        for i in range(n_players):
            mstat.append(f"{i+1},m{gw},{45 + (i % 3) * 30},0,0,0.3,0.1")
        (folder / "playermatchstats.csv").write_text(
            "\n".join(mstat) + "\n", encoding="utf-8")
