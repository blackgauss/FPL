"""API contract tests over a synthetic data root (tmp Store, no disk data)."""
...
def test_headless_render_smoke(client: TestClient, tmp_path: Path,
                               monkeypatch: pytest.MonkeyPatch) -> None:
    fake_forecast(monkeypatch)
    r = render_smoke(client, tmp_path / "payloads.json")
...
def test_clock_single_source_of_gw_truth(client: TestClient,
    ...
    assert client.app.state.store.clock() == {"current": 1, "next": 2,
                                              "scoreable": 2}
    bare = Store(root=tmp_path / "nothing-here")
    assert bare.clock() == {"current": 0, "next": 1, "scoreable": 0}
